//taliyam123@gmail.com
#include "SquareMat.hpp"
using namespace matrix;
//constructor
SquareMat::SquareMat(int n,double initVal){
    if(n<=0){
        throw std::invalid_argument("the size of the matrix must be positive");
    }
    this->n= n;
    this->mat = new double*[n];
    for(int i=0;i<n;i++){
        this->mat[i]=new double[n];
        for(int j=0;j<n;j++){
            this->mat[i][j]=initVal;
        }
    }
}
//copy constructor
SquareMat::SquareMat(const SquareMat& other){
    this->n=other.n;
    this->mat = new double*[n];
    for(int i=0;i<n;i++){
        this->mat[i]=new double[n];
        for(int j=0;j<n;j++){
            this->mat[i][j]=other.mat[i][j];
        }
    }
}
//destructor
SquareMat::~SquareMat(){
    for(int i=0;i<this->n;i++){
        delete[] this->mat[i];
    }
    delete[] this->mat;
}
//mat1 + mat2
SquareMat SquareMat::operator+(const SquareMat& other){
    if(this->n!=other.n){
        throw std::invalid_argument("The sizes of the two matrix should be the same");
    }
    SquareMat newMat(*this);
    for (int i = 0; i < this->n; i++)
    {
        for(int j=0;j< this->n;j++){
            newMat.mat[i][j]+=other.mat[i][j];
        }
    }
    return newMat;
}

//mat1 - mat2
SquareMat SquareMat::operator-(const SquareMat& other){
    if(this->n!=other.n){
        throw std::invalid_argument("The sizes of the two matrix should be the same");
    }
    SquareMat newMat(*this);
    for (int i = 0; i < this->n; i++)
    {
        for(int j=0;j< this->n;j++){
            newMat.mat[i][j]-=other.mat[i][j];
        }
    }
    return newMat;
}

//mat2 = -mat1
SquareMat SquareMat::operator-(){
    SquareMat newMat(this->n);
    for (int i = 0; i < this->n; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            newMat.mat[i][j]=-this->mat[i][j];   
        }
    }
    return newMat;
}

//mat1 * mat2
SquareMat SquareMat::operator*(const SquareMat& other){
    if(this->n!=other.n){
        throw std::invalid_argument("The sizes of the two matrix should be the same");
    }
    SquareMat newMat(this->n);
    for (int i = 0; i < this->n; i++)
    {
        for(int j=0;j<other.n;j++){
            for(int k=0;k<this->n;k++){
                newMat.mat[i][j]+=((this->mat[i][k])*(other.mat[k][j]));
            }
        }
    }
    return newMat;
}

//matrix * scalar
SquareMat SquareMat::operator*(double scalar){
    SquareMat newMat(*this);
    for (int i = 0; i < this->n ; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            newMat.mat[i][j]*=scalar;
        }
    }
    return newMat;
}

//scalar * matrix
SquareMat matrix::operator*(double scalar,SquareMat& mat){
    return mat*scalar;//because we already implemented this operator. 
}

//mat1 % mat2 - mulitple between every two items on the same position
SquareMat SquareMat::operator%(const SquareMat& other){
    if(this->n!=other.n){
        throw std::invalid_argument("The sizes of the two matrix should be the same");
    }
    SquareMat newMat(*this);
    for (int i = 0; i < this->n; i++)
    {
        for(int j=0;j< this->n;j++){
            newMat.mat[i][j]*=other.mat[i][j];
        }
    }
    return newMat;
}

//matrix % scalar
SquareMat SquareMat::operator%(int scalar){
    SquareMat newMat(this->n);
    for (int i = 0; i < this->n; i++)
    {
        for(int j=0;j< this->n;j++){
            newMat.mat[i][j]=fmod(this->mat[i][j],scalar);
        }
    }
    return newMat;
}

//matrix/scalar
SquareMat SquareMat::operator/(double scalar){
    SquareMat newMat(*this);
    for (int i = 0; i < this->n; i++)
    {
        for(int j=0;j< this->n;j++){
            newMat.mat[i][j]/=scalar;
        }
    }
    return newMat;
}

//matrix ^ scalar
SquareMat SquareMat::operator^(int scalar){
    if(scalar<0){
        throw std::invalid_argument("the scalar must be bigger or equal to 0");
    }

    if(scalar==0){
        SquareMat newMat(this->n);
        for (int i = 0; i < this->n; i++)
        {
            newMat.mat[i][i]=1;
            //the other items are all 0 from the constructor
        }
        return newMat;
    }
    SquareMat newMat(*this);

    for (int i = 1; i < scalar; i++)
    {
       newMat*=(*this);
    }
    return newMat;
    
}

// matrix++;
SquareMat SquareMat::operator++(int dummy_flag_for_postfix_increment){//n++
    SquareMat newMat(*this);
    for (int i = 0; i < this->n; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            this->mat[i][j]++;
        }
    }
    return newMat;
    
}

//++matrix
SquareMat& SquareMat::operator++(){//++n
    for (int i = 0; i < this->n; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            this->mat[i][j]++;
        }
    }
    return *this;
}

//matrix --
SquareMat SquareMat::operator--(int dummy_flag_for_postfix_increment){//n--
    SquareMat newMat(*this);
    for (int i = 0; i < this->n; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            this->mat[i][j]--;
        }
    }
    return newMat;
    
}

//--matrix
SquareMat& SquareMat::operator--(){//--n
    for (int i = 0; i < this->n; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            this->mat[i][j]--;
        }
    }
    return *this;
}

//~matrix - Transpose
// switch a[i][j] with a[j][i]
SquareMat SquareMat::operator~(){
    SquareMat transposeMat(this->n,this->n);
    for (int i = 0; i < this->n; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            transposeMat[j][i]=this->mat[i][j];
        }
    }
    return transposeMat;
}

// mat[i] - give acsses to the item in the position [i][j] with mat[i][j] 
double* SquareMat::operator[](int index){
    if(index<0||this->n<=index){
        throw std::invalid_argument("The index must be bewteen 0 to rows -1");
    }
    return this->mat[index];
}

// mat1 == mat2
//return true if the sum of all the values in both matrix are equals
bool SquareMat::operator==(const SquareMat& other){
    double sum1=0, sum2=0;
    for (int i = 0; i < this->n; i++)
    {
        for (int j = 0; j < this->n; j++)
        {
            sum1+=this->mat[i][j];
        }
    }
    for (int i = 0; i <other.n; i++)
    {
        for (int j = 0; j < other.n; j++)
        {
            sum2+=other.mat[i][j];
        }
    }
    if(sum1!=sum2){
        return false;
    }
    return true;
}


//mat1 != mat2
bool SquareMat::operator!=(const SquareMat& other){
    return !(*this==other);
}

// mat1 <= mat2
bool SquareMat::operator<=(const SquareMat& other){
    return((*this)<other || (*this)==other);
}

//mat1 < mat2
bool SquareMat::operator<(const SquareMat& other){
    double sum1=0, sum2=0;
        for (int i = 0; i < this->n; i++)
        {
            for (int j = 0; j < this->n; j++)
            {
                sum1+=this->mat[i][j];
            }
        }
        for (int i = 0; i <other.n; i++)
        {
            for (int j = 0; j < other.n; j++)
            {
                sum2+=other.mat[i][j];
            }
        }
        if(sum1<sum2){
            return true;
        }
        return false;
}

//mat1 >= mat2
bool SquareMat::operator>=(const SquareMat& other){
    return((*this)>other || (*this)==other);
}

//mat1 > mat2
bool SquareMat::operator>(const SquareMat& other){
    return !((*this)<=other);
}

// !matrix - calculate the det of the matrix
// a recursive function that stop when the size of the matrix is 1x1 or 2x2
double SquareMat::operator!(){
    if(this->n==1){
        return mat[0][0];
    }
    if(this->n==2){
        return(mat[0][0]*mat[1][1]-mat[0][1]*mat[1][0]);
    }
    SquareMat temp(this->n-1,this->n-1);
    double det=0;
    for(int k=0;k<this->n;k++){
        for(int i=1;i<this->n;i++){
            for(int j=0;j<this->n;j++){
                if(k<j){
                    temp[i-1][j-1]=mat[i][j];
                }
                if(k>j){
                    temp[i-1][j]=mat[i][j];
                }
            }
        }
        //recursive call with matrix with size n-1
        double tempSum=!temp*mat[0][k];
        //(-1)^k
        if(k%2==0){//k is even
            det+=tempSum;
        }
        else{//k is odd
            det-=tempSum;
        }
    }
    return det;
}

// mat1%=mat2;
SquareMat SquareMat::operator%=(const SquareMat& other){
    *this = (*this)%other;
    return (*this);
}

//mat1%=scalar;
SquareMat SquareMat::operator%=(int scalar){
    *this=*this%scalar;
    return(*this);
}

//mat1*=mat2;
SquareMat SquareMat::operator*=(const SquareMat& other){
    *this =( (*this)*other);
    return (*this);
}

//mat1*=scalar
SquareMat SquareMat::operator*=(double scalar){
    *this = (*this)*scalar;
    return (*this);
}

//mat1 -= mat2;
SquareMat SquareMat::operator-=(const SquareMat& other){
    *this = (*this) - other;
    return (*this);
}

//mat1 /= scalar;
SquareMat SquareMat::operator/=(const double scalar){
    *this = (*this)/scalar;
    return (*this);
}

//mat1 += mat2;
SquareMat SquareMat::operator+=(const SquareMat& other){
    *this = (*this)+other;
    return (*this);
}


//cout<<mat1;
//writing to output and than he will be printed in the cout.
std::ostream& matrix::operator<< (std::ostream& output, const SquareMat& mat){
    for(int i=0;i<mat.n;i++){
        for(int j=0;j<mat.n;j++){
            output<<mat.mat[i][j]<<" ";
        }
        output<<std::endl;
    }
    return output;
}

// mat1.getN();
int SquareMat::getN(){
    return this->n;
}

//mat1 = mat2;
SquareMat& SquareMat::operator=(const SquareMat& other){

    if (this==&other)
        return *this;
    if(this->n == other.n){
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                this->mat[i][j]=other.mat[i][j];
            }
        }
    }
    else{
        for(int i=0;i<n;i++){
            delete[] this->mat[i];
        }
        delete[] this->mat;
        this->n = other.n;
        this->mat = new double*[n];
        for(int i=0;i<n;i++){
            this->mat[i]=new double[n];
            for(int j=0;j<n;j++){
                this->mat[i][j]=other.mat[i][j];
            }
        }
    }
    return *this;
}